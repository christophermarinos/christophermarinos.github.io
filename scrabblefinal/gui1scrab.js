...
function test_func(){//was originally a test function but it really turned into the function which inits all scrabble pieces
 ...
 function construct_piece(_worldx,_worldy,_has_return_spot,_homex,_homey,_imgval,_wid,_ht,_value)//I use this function to do piece assignment. It grew significantly over the course of the project.
{
	...
	return _scrabpc;
	  ...
	  function drawpieces()//I am proud that this did not get too cluttered. There are only two arrays to draw, potential matches and permanently placed pieces.
	  {
	  ...
	  function click_up_coll(x,y){
		  ...
		  if((x > BOARD_GLOB_POSX && x < BOARD_GLOB_POSX + boardwid)&&(y > BOARD_GLOB_POSY && y < BOARD_GLOB_POSY + boardht))//if we dropped on the board
		  {
			  ...
			  if(globarrplacedonboard.length == 0)//if nothing has been placed on the board this round yet, special case
			  {
		  ...
		  if((x > TRAY_START && x < TRAY_START + AVG_IMG_WID * PIECES_IN_TRAY)&&(y > rect.bottom - AVG_IMG_HT))//collide with the tray?
		  {console.log('HIT');
			for(i = 0; i < globarrplaced.length; i++)
			{
				if(i != CURR_PIECE)//dont collide with myself; this is super important and rears its head repeatedly in this code
				{
	...
	//validates the current pieces on the board returns true if they are all contiguous on an x or y axis otherwise false  
	  ...
	  function dictionary_check(str){return true;}
